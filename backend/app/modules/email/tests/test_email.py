...
def test_send_templated_email_writes_history(client) -> None:
    ...
    history = client.get("/emails/history", params={"to_email": "user@example.com"})
...
def test_screenshot_upload_and_notify_gates(client, tmp_path) -> None:
    ...
    upload = _upload_csv(client, sample)
    ...
    with (
        ...
        blocked = client.post(f"/batches/rows/{row_id}/notify/redeem")
        ...
        with patch("app.modules.email.services.send_email.send_raw_email") as mocked:
            mocked.return_value = None
            redeem_mail = client.post(f"/batches/rows/{row_id}/notify/redeem")
        ...
        order_blocked = client.post(f"/batches/rows/{row_id}/notify/order")
        ...
        with patch("app.modules.email.services.send_email.send_raw_email") as mocked:
            mocked.return_value = None
            order_mail = client.post(f"/batches/rows/{row_id}/notify/order")
